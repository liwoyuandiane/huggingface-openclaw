...
### 方案一：使用 HuggingFace 免费模型（推荐）

只需要 HuggingFace Token 即可使用免费的 DeepSeek-R1 等模型：
...
# 系统会自动使用 huggingface/deepseek-ai/DeepSeek-R1 免费模型
```

### 方案二：使用 ModelScope（推荐 - 免费）
...
AUTO_CREATE_DATASET=true            # 自动创建 Dataset
HF_SPACE_DOMAIN=my-space            # Space 名称
OPENROUTER_API_KEY=sk-or-xxx       # OpenRouter API Key（在 openrouter.ai 获取）
OPENCLAW_GATEWAY_PASSWORD=admin123  # 网关密码
```
...
**方案一：HuggingFace 免费模型（最简配置）**

只需要 HuggingFace Token 即可使用免费模型：
...
| `OPENCLAW_DEFAULT_MODEL` | 默认模型（可选） | `huggingface/deepseek-ai/DeepSeek-R1` |
...
本项目会自动将环境变量传递给 OpenClaw 进程，让你可以在一个 Space 中同时使用多个 API 提供商。

### 支持的环境变量

本项目自动传递以下环境变量给 OpenClaw：

| 环境变量 | 说明 | 支持的提供商 |
|----------|------|-------------|
| `HF_TOKEN` | HuggingFace Token | HuggingFace Inference |
| `OPENAI_API_KEY` | OpenAI 兼容 API Key | OpenAI、SiliconFlow、Any API |
| `OPENAI_API_BASE` | OpenAI 兼容 API 地址 | - |
| `MODELSCOPE_API_KEY` | ModelScope Token | ModelScope |
| `OPENROUTER_API_KEY` | OpenRouter API Key | OpenRouter (200+ 模型) |
| `ANTHROPIC_API_KEY` | Anthropic Claude Key | Claude |
| `GOOGLE_API_KEY` | Google Gemini Key | Gemini |
...
| 环境变量 | 说明 | 示例值 |
|----------|------|--------|
| `OLLAMA_HOST` | Ollama 本地/远程地址 | `http://localhost:11434` |
| `OLLAMA_NUM_PARALLEL` | 并行数限制 | `3` |
| `OLLAMA_KEEP_ALIVE` | 模型保持加载时间 | `5m` |
| `OPENCLAW_HTTP_PROXY` | HTTP 代理 | `http://proxy:8080` |
| `OPENCLAW_HTTPS_PROXY` | HTTPS 代理 | `http://proxy:8080` |
| `OPENCLAW_NO_PROXY` | 跳过代理的地址 | `localhost,127.0.0.1` |

### 内存存储配置

| 环境变量 | 说明 | 示例值 |
|----------|------|--------|
| `OPENCLAW_MEMORY_BACKEND` | 记忆后端 | `sqlite` |
| `OPENCLAW_REDIS_URL` | Redis 地址 | `redis://localhost:6379` |
| `OPENCLAW_SQLITE_PATH` | SQLite 路径 | `/data/.openclaw/memory.db` |

### 多 API 组合使用示例

你可以在一个 Space 中配置多个 API：

```bash
# 主要配置
HF_TOKEN=hf_xxx
OPENROUTER_API_KEY=sk-or-xxx  # 主模型（通过 OpenRouter）

# 备用配置（可选）
MODELSCOPE_API_KEY=ms-xxx      # 备用模型（ModelScope）
ANTHROPIC_API_KEY=sk-ant-xxx   # Claude 备用
GOOGLE_API_KEY=AIzaSyxxx       # Gemini 备用
```

OpenClaw 会根据模型名称自动选择对应的 API 提供商。
